...
import { YouTubeRelatedSource } from './services/youtubeRelated.js';
import { CatalogService } from './catalog/catalog.js';
import { ConvexCoverStorage, ConvexGrantLedger, ConvexRelationStore, ConvexUserStore } from './db/convex.js';
...
export function createServices(options: ServiceOptions): AppServices {
  ...
  return {
    ...
    recommendations: new RecommendationService(catalog, cache, options.youtubeMusicApiUrl
      ? new YouTubeRelatedSource(new YouTubeMusicProvider({
          baseUrl: options.youtubeMusicApiUrl,
          ...(options.fetchImpl ? { fetchImpl: options.fetchImpl } : {})
        }), catalog, cache)
      : undefined,
    // Song relations outlive a cold start only in Convex; without it they are per-instance memory.
    options.convexUrl && options.convexServerSecret
      ? new ConvexRelationStore({ url: options.convexUrl, serverSecret: options.convexServerSecret })
      : undefined),
    ...(options.convexUrl && options.convexServerSecret
